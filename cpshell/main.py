...
class DeviceSerial(Device):
  ...
  def __init__(self, port, baud, wait):
    ...
    for _ in range(20):
      ...
      data = cpb.read_until(1, b'>>> ', timeout=0.1)
      if data.endswith(b'>>> '):
        connected = True
        break
      if not QUIET:
...
class Shell(cmd.Cmd):
  ...
  def do_boards(self, _):
    """boards
      ...
      Lists the boards that cpshell is currently connected to.
    """
    rows = []
    with DEV_LOCK:
      for dev in DEVS:
